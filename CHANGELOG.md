...
## 3.6.0 — Memória da Empresa
- Nova aba "Arquivos e artes" dentro do cadastro/edição de cada produto.
- Upload individual conforme os arquivos forem encontrados ou criados.
- Suporte a fotos, PDFs, SVG, STL, ZIP, vídeos e demais arquivos de produção.
- Classificação por tipo, tags e descrição curta.
- Arquivo mestre/favorito.
- Arquivamento e restauração sem exclusão definitiva.
- Pesquisa do catálogo também encontra nomes, tags e descrições dos anexos.
- Arquivos salvos no Supabase Storage, com fallback local.
