...
def upload_memory_file(upload: Any, folder: str = "memoria", local_upload_dir: str = "uploads/memoria") -> str:
    """Envia qualquer arquivo da memória ao bucket público ``catalogo``.

    O bucket já é usado pelo catálogo, evitando uma configuração extra no
    Supabase. Se o envio online falhar, mantém uma cópia local como fallback.
    """
    if upload is None:
        return ""

    safe_name = re.sub(r"[^A-Za-z0-9._-]", "_", str(upload.name))
    safe_folder = re.sub(r"[^A-Za-z0-9/_-]", "_", str(folder).strip("/")) or "memoria"
    unique_name = f"{datetime.now().strftime('%Y%m%d%H%M%S%f')}_{safe_name}"
    object_name = f"{safe_folder}/{unique_name}"
    content = bytes(upload.getbuffer())
    content_type = getattr(upload, "type", None) or "application/octet-stream"
    ...
    local_base = Path(local_upload_dir) / safe_folder
    local_base.mkdir(parents=True, exist_ok=True)
    local_path = local_base / unique_name
    local_path.write_bytes(content)
    return str(local_path).replace("\\", "/")
