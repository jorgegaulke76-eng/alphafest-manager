"""HF48 — camada visual comercial do site AlphaFest (HF51.4-HF3 · base visual HF48.3-HF4).
...
ICONES_CATEGORIA_ARQUIVOS = {
    "balao": "balao.webp",
    "bandeira": "bandeiras_flamulas.webp",
    "flamula": "bandeiras_flamulas.webp",
    "centro de mesa": "centro_mesa.webp",
    "convite": "convite.webp",
    "copo": "copos_canecas.webp",
    "caneca": "copos_canecas.webp",
    "embalagem": "embalagem.webp",
    "laser": "gravacao_laser.webp",
    "graf": "grafica.webp",
    "3d": "impressao_3d.webp",
    "kit festa": "kit_festa.webp",
    "lembranca": "lembrancas.webp",
    "papel de arroz": "papel_arroz.webp",
    "papelaria": "papelaria.webp",
    "personal": "personalizados.webp",
}
...
def _category_icon_data_uri(nome_arquivo: str) -> str:
    caminho = Path(__file__).resolve().parent / "assets" / "site" / "category_icons" / str(nome_arquivo or "")
    try:
        dados = caminho.read_bytes()
    except OSError:
        return ""
    return f"data:image/webp;base64,{base64.b64encode(dados).decode('ascii')}"


def _category_icon_html(nome_arquivo: str) -> str:
    uri = _category_icon_data_uri(nome_arquivo)
    if not uri:
        return "🎈"
    return f'<img class="hf48-cat-icon-img" src="{uri}" alt="" aria-hidden="true">'


def _icone_categoria(nome: str) -> str:
    chave = _slug(nome).replace("-", " ")
    arquivo = ""
    # Casos mais específicos primeiro para evitar que PAPEL DE ARROZ caia em PAPELARIA.
    for termo, candidato in ICONES_CATEGORIA_ARQUIVOS.items():
        if termo in chave:
            arquivo = candidato
            break
    if not arquivo:
        arquivo = "personalizados.webp"
    uri = _category_icon_data_uri(arquivo)
    if not uri:
        return "⭐"
    return f'<img class="hf48-cat-icon-img" src="{uri}" alt="" aria-hidden="true">'
    ...
    *,
    ...
    for i, item in enumerate(escolhidos):
        ...
        slides.append(
            ...
            f'<div class="hf50-carousel-media">{img_html}</div>'
            f'<div class="hf50-carousel-copy"><h2>{html.escape(nome)}</h2>'
            f'<div class="hf50-carousel-actions"><button type="button" class="hf50-carousel-product" data-hf50-product="{html.escape(nome, quote=True)}">Ver produto</button></div></div>'
            f'</article>'
        )
...
def _categorias_html(catalogo: Iterable[Dict[str, Any]], galeria: Iterable[Dict[str, Any]] | None = None) -> str:
    ...
    if mostrar_ocasioes:
        cards.append(
            ...
            f'<span class="hf48-cat-icon">{_category_icon_html("datas_ocasioes.webp")}</span>'
...
.hero-card.hf48-mascot-hero{min-height:430px;display:block;padding:34px 52% 28px 22px;background:transparent;border:0;border-radius:0;box-shadow:none;backdrop-filter:none;overflow:visible}
.hero-card.hf48-mascot-hero:after{display:none}
.hf48-mascot-hero .hf48-mascot-copy{position:relative;z-index:4;max-width:305px}.hf48-mascot-hero .hf48-mascot-copy p{position:relative;z-index:4;font-size:16px}
.hf48-mascot-hero .hf48-mascot-copy h2{font-size:30px;line-height:1.05}
.hf48-hero-mascot-img{position:absolute;right:-3%;bottom:-18px;width:62%;max-height:455px;object-fit:contain;z-index:3;filter:drop-shadow(0 18px 26px rgba(18,35,61,.16))}
...
.hf50-carousel{background:transparent;padding:0 24px 24px;margin:0;position:relative;z-index:4}.hf50-carousel-shell{max-width:1480px;margin:auto;position:relative;overflow:hidden;border:1px solid #dbe9f6;border-radius:25px;background:linear-gradient(115deg,#eefaff 0%,#fff 45%,#fff1f8 76%,#fff9df 100%);box-shadow:0 14px 38px rgba(18,35,61,.08);padding:18px 52px 22px}.hf50-carousel-viewport{overflow:hidden;position:relative;z-index:2}.hf50-carousel-track{display:flex;gap:14px;transition:transform .46s cubic-bezier(.2,.75,.25,1);will-change:transform}.hf50-carousel-slide{flex:0 0 calc((100% - 42px)/4);min-width:0;position:relative;border:1px solid #e0eaf3;border-radius:22px;overflow:hidden;min-height:300px;padding:0;display:flex;flex-direction:column;background:#fff;box-shadow:0 8px 22px rgba(18,35,61,.07)}.hf50-carousel-media{position:relative;inset:auto;z-index:1;background:#fff;border:0;border-radius:0;height:230px;box-shadow:none;overflow:hidden}.hf50-carousel-product-img{display:block;width:100%;height:100%;object-fit:cover;padding:0;opacity:1!important;filter:none!important}.hf50-carousel-slide:after{display:none!important;content:none!important}.hf50-carousel-copy{position:relative;z-index:2;padding:14px 16px 16px;max-width:none;color:#12233d;background:#fff;display:flex;flex-direction:column;gap:10px;flex:1}.hf50-carousel-copy h2{font-size:19px;line-height:1.12;color:#12233d;margin:0;text-shadow:none;font-weight:900}.hf50-carousel-tax,.hf50-carousel-copy p,.hf50-carousel-whatsapp{display:none!important}.hf50-carousel-actions{display:flex;gap:7px;flex-wrap:wrap;margin-top:auto}.hf50-carousel-actions button{border:1px solid #cfe2f3;border-radius:11px;padding:10px 13px;font-size:12px;font-weight:900;cursor:pointer}.hf50-carousel-product{background:#fff;color:#0875d4}.hf50-carousel-product:hover{background:#eef8ff}.hf50-carousel-placeholder{display:flex;align-items:center;justify-content:center;width:100%;height:100%;font-size:20px;font-weight:950;color:#0875d4;background:#eef8ff}.hf50-carousel-fox{display:none}.hf50-carousel-arrow{position:absolute;top:50%;transform:translateY(-50%);z-index:6;width:38px;height:38px;border-radius:50%;border:1px solid #cfe2f3;background:rgba(255,255,255,.96);color:#0875d4;font-size:27px;line-height:1;cursor:pointer;box-shadow:0 6px 18px rgba(18,35,61,.12)}.hf50-carousel-arrow.prev{left:8px}.hf50-carousel-arrow.next{right:8px}.hf50-carousel-dots{position:relative;z-index:7;display:flex;justify-content:center;gap:6px;margin-top:14px}.hf50-carousel-dot{width:8px;height:8px;border:0;border-radius:50%;background:#b8cee0;padding:0;cursor:pointer}.hf50-carousel-dot.active{width:23px;border-radius:999px;background:linear-gradient(90deg,#0875d4,#ff2f91)}
...
(function(){
  ...
  // HF51.4-HF1 — carrossel em loop infinito real: autoplay sempre avança no mesmo sentido.
