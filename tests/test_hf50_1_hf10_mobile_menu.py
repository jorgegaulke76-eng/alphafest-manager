...
def test_mobile_more_contract():
    assert 'HF51.1-HF1' in SRC
    assert 'hf50-mobile-more' in SRC
    assert "new Set(['Serviços','Quem Somos','Contato'])" in SRC
...
def test_version_and_production_contract():
    assert 'HF51.1-HF1' in APP
    assert Path('VERSAO.txt').read_text(encoding='utf-8').strip() == '20.4.9-I8.13.5-HF51.1-HF1'
