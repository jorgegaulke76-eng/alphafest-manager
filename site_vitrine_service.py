...
def gerar_html_vitrine(
    ...
    *,
    ...
    texto_escolha = "Pesquise por nome, material ou ocasião — ou escolha uma categoria e depois uma subcategoria." if usar_taxonomia_catalogo else "Pesquise ou escolha uma categoria."
...
<main class="main" id="produtos"><div class="section-head"><div><h2>Encontre seu personalizado</h2><p>{html.escape(texto_escolha)}</p></div><div id="result-count">{resumo['total']} produto(s)</div></div><div class="toolbar"><label class="search"><input id="search" type="search" placeholder="Buscar produto, material, ocasião, categoria..."></label></div>{('<div class="taxonomy-nav"><section class="taxonomy-focus" id="taxonomy-focus" hidden><button type="button" class="taxonomy-back" id="taxonomy-back">← Voltar às categorias</button><div class="taxonomy-focus-copy"><small>Categoria escolhida</small><strong id="taxonomy-focus-label">Categoria</strong></div></section><section class="taxonomy-step taxonomy-cats" id="taxonomy-cats"><div class="taxonomy-heading"><div><span class="step-number">1</span><strong>Escolha uma categoria</strong></div><small>Selecione uma categoria para ver somente os produtos e subcategorias dela.</small></div><div class="filters">' + ''.join(chips) + '</div></section>' + subfilters_html + '<div class="taxonomy-current" id="taxonomy-current"><strong>Todos os produtos</strong><span>Escolha uma categoria para ver as subcategorias.</span></div></div>') if usar_taxonomia_catalogo else ('<div class="filters">' + ''.join(chips) + '</div>')}<div class="grid" id="grid">{''.join(cards)}</div>{vazio}</main>
